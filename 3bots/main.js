class board{
	...
	try{
        ...
        if(JSON.stringify(bot0move)!=JSON.stringify(bot1move)&&JSON.stringify(bot1move)!=JSON.stringify(bot2move)&&JSON.stringify(bot2move)!=JSON.stringify(bot0move)){
            try{
...
boardgame=new board(50,50,[BrainlessBob,BrainlessBob,BrainlessBob])
ticks=0
targetTicks=3000
BOT1=BrainlessBob
BOT2=BrainlessBob
...
tickfunction=()=>{
    ...
    document.getElementById("score").innerHTML=`${b2}<br>${b1}<br>${b0}<br>Days:${ticks}`
    if(c[0]>c[1]&&c[0]>c[2]){
    ...
    if(ticks>=targetTicks){
        ...
        if(c[0]>c[1]&&c[0]>c[2]){
            document.getElementById("score").style.backgroundColor="rgb(230,131,28)"
            document.getElementById("score").innerHTML=`${boardgame.bots[0].name} (Orange) won!<br>${b2}<br>${b1}<br>${b0}<br>Days:${ticks}`
        }else if(c[0]<c[1]&&c[1]>c[2]){
            document.getElementById("score").style.backgroundColor="rgb(117,212,135)"
            document.getElementById("score").innerHTML=`${boardgame.bots[1].name} (Green) won!<br>${b2}<br>${b1}<br>${b0}<br>Days:${ticks}`
        }else if(c[1]<c[2]&&c[2]>c[0]){
	    document.getElementById("score").style.backgroundColor="rgb(217,102,253)"
	    document.getElementById("score").innerHTML=`${boardgame.bots[2].name} (Purple) won!<br>${b2}<br>${b1}<br>${b0}<br>Days:${ticks}`
	}else{
            document.getElementById("score").style.backgroundColor="wheat"
            document.getElementById("score").innerHTML=`Tie!<br>${b2}<br>${b1}<br>${b0}<br>Days:${ticks}`
        }
    }
